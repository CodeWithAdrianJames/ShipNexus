...
| AWS_ACCESS_KEY_ID      | test                                             | LocalStack credentials; omit in ECS to use the task role                                                           |
| AWS_SECRET_ACCESS_KEY  | test                                             | LocalStack credentials; omit in ECS to use the task role                                                           |
| SQS_QUEUE_NAME         | shipnexus-deployments.fifo                       | Must match the queue created in LocalStack or AWS                                                                  |
| SQS_VISIBILITY_TIMEOUT | 900                                              | Seconds. Must exceed expected job processing time. Redis lock TTL is derived from this value plus a safety margin. |
...
- failed: unrecoverable error; errorMessage written; SQS message deleted
  because the database row is the durable terminal record

Monitor failed rows in Postgres and the SQS
